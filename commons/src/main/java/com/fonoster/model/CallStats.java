...
public class CallStats {
  ...
  public LinkedHashMap<String, TrafficInfo> getStats() {
    ...
    if (getPeriod().equals(Period.DAY)) {
      for (String k : keys) {
        if (DateTime.now()
            .minusHours(getSamplesFor(Period.DAY))
            .isBefore(new BigInteger(k).longValue())) {
          s.put(k, stats.get(k));
        }
      }
    ...
    if (getPeriod().equals(Period.MONTH)) {
      for (String k : keys) {
        if (DateTime.now()
            .minusDays(getSamplesFor(Period.MONTH))
            .isBefore(new BigInteger(k).longValue())) {
          s.put(k, stats.get(k));
        }
      }
  ...
  public String getKeyFor(Period period) {
    ...
    if (period.equals(Period.REALTIME)) {
      dt = DateTime.now().withMillisOfSecond(0);
    } else if (period.equals(Period.HOUR)) {
      dt = DateTime.now().withSecondOfMinute(0).withMillisOfSecond(0);
    } else if (period.equals(Period.DAY)) {
      dt = DateTime.now().withMinuteOfHour(0).withSecondOfMinute(0).withMillisOfSecond(0);
