...
public class FonosterJSTest {
  ...
  public void testSay() throws Exception {
    ...
    when(ttsFactory.getTTSEngine("default").generate("allison", "pressed nothing"))
        .thenReturn("test2");
  ...
  public void testGatherPressNothing() throws Exception {
    // Mockito stubbing
    AstivletResponse response = mock(AstivletResponse.class);
    when(response.getOption("silence/10", "1234567890#*", 5000)).thenReturn('\0');
  ...
  public void testGatherPressFewDigits() throws Exception {
    // Mockito stubbing
    AstivletResponse response = mock(AstivletResponse.class);
    when(response.getOption("silence/1", "1234567890#*", 0)).thenReturn('1').thenReturn('2');
  ...
  public void testRecord() throws Exception {
    ...
    when(response.recordFile(
            commonsConfig.getRecordingsPath().concat("/") + record.getId(),
            "wav",
            "1234567890*#",
            3600 * 1000,
            0,
  ...
  public void testLoadJS() throws Exception {
    ...
    engine.eval(
        "print($http)",
        engineScope); // ReferenceError: "$http" is not defined wil be launch if it fails
